 (function ($) {
  ...
  /*
   Possible values are other, prepend, append
   */
  writeType: "other"
  };
  ...
  function translator(elem, val) {
    //first we'll see if the val is an object
    //if so we iterate through and load the attributes
    if (typeof val === 'object') {
        for (v in val) {
        //note we don't care what attributes get loaded.
        $(elem).attr(v, val[v]);
        }
    }
    else {
  ...
  //set context if it needs to be proxied to the next method.
  if (ctx) {
